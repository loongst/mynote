...
class Monitordb:
    ...
    def query(self,collection_key,query_key,query_list,query_db='hydro'):
        db=self.client[query_db]
        collection=db[collection_key]
        result=[]
        for x in query_list:
            querys={query_key,x}
            result.append(collection.find(querys))
        return result
        
    def get_record(self):
        items={}
        db = self.client['hydro']
        ...
        for pid in self.pids:
            for uid in self.users.values():
                ...
                if len(items[pid][uid])>1:
                    ...
                    endtime=datetime.datetime(2023, 1, 1, 2, 50, 55, 512000)
                    endx=''
                    for x in cleans:
    ...
    def get_storage_byrecord(self,items):
        items=items
        db = self.client['hydro']
        collection=db['storage']
        print("-"*200)
        for pid in self.pids:
            for uid in self.users.values():
                # print(type(items[pid][uid]))
                tt=items[pid][uid]
                if tt:
                    # print(tt)
                    subfile=tt[0]["files"]["code"]
                    subpath,subname=subfile.split('#')
                    print(subpath,subname) 

                    query = {'path': "submission/"+subpath}
                    print(query)
                    results = collection.find(query)
    ...
    def move_request(self,abpath,subname,owner):
        tmppath=os.path.basename(abpath)
        os.mkdir("/tmp/{}".format(tmppath))
        shutil.copy(abpath,"/tmp/{}/{}".format(tmppath,subname))
        shutil.unpack_archive("/tmp/{}/{}".format(tmppath,subname),"/tmp/{}/".format(tmppath))



my_monitor=Monitordb()

# my_monitor.get_storage()
my_monitor.get_storage_byrecord(my_monitor.get_record())
